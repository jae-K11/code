...
$('header .gnb .gnb_wrap ul.depth1 > li').on('mouseenter focusin',function(){
    if(device_status == 'pc'){ //pc일때만 실행
         ...
         그러면 이전에 오버했던 li를 정확하게 몰라도 된다 
         --> 원래 over클래스가 없던 요소는 removeClass 줘도 아무일도 안일어남 */
         $('header .gnb .gnb_wrap ul.depth1 > li').removeClass('over')
         $(this).addClass('over')
    }
})

/*메뉴는 오버를 감지하는 영역보다 out을 잡아주는 영역이 훨씬 커야함 */
...
/*
    ...
    2. 열려있는 메뉴를 다시 클릭하면 닫힘 removeClass
    3. 이미 하나가 열려있는 상태에서 다른 메뉴를 클릭하면
       ...
       ----> 메뉴를 클릭했을때 
           ...
           1차 메뉴 li에 open 클래스가 있으면 열려있는것...
           hasClass 

    4. 모바일에서 1차메뉴를 클릭하면 1차메뉴 a에 입력되어 있는 링크주소로 이동
       메뉴를 안열고..
       a태그에 있는 href로 페이지 이동되는것을 막아햐함
       ----> a에 클릭했을때 선택자 변경 
       $(this)가 a가 됨 .. 근데 난 li를 선택해야함
...
$('header .gnb .gnb_wrap ul.depth1 > li > a').on('click', function(e){
    if(device_status == 'mobile'){
        ...
        if(depth1_open == true){ //열려있을 경우에 , ==는 비교연산자(같다)
